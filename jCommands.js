...
function createLskTx(){
	...
	var transaction = onz.transaction.createTransaction(receiver, amount, pass, second);
...
function createSecPassTx(){
	...
	second = second.trim();
	var transaction = onz.signature.createSignature(pass, second);
...
function createVotingTx(){
	...
	deleg = deleg.trim();
	var transaction = onz.vote.createVote(pass, deleg.split(",") , second);
...
function createDelegateTx(){
	...
	var transaction = onz.delegate.createDelegate(pass, degName, second);
...
function getNewAccount(){
	...
	var keys = onz.crypto.getKeys(newPassphrase);
	var pubKey = keys['publicKey'];
	var privKey = keys['privateKey'];
	var accountAddress = onz.crypto.getAddress(pubKey);
...
function getRandomPassphrase(){
	...
	var sha256Hash = onz.crypto.getSha256Hash(lPassphrase128,"hex");
